...
async function assignLicense(req, res) {
  const { licenseKey, name, assigned_to } = req.body;
  const issued_by = req.email
  // console.log(licenseKey);
  
   const connection = db.pool();
  
  if (!licenseKey || !assigned_to || !name) {
  ...
  try {
    ...
    const [result] = await connection.execute(
      `UPDATE licenses
       SET assigned_to = ?, name = ?,issued_by = ?, assigned_status = 1, updated_at = NOW()
       WHERE license_key = ?`,
      [assigned_to, name,issued_by ,licenseKey]
    );
